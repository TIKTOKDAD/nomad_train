...
class NoMaD_ViNT(nn.Module):
    ...
    def __init__(
        ...
        # no_mask: 全0，不屏蔽任何token（导航模式）
        self.goal_mask = torch.zeros((1, self.context_size + 2), dtype=torch.bool)
        self.goal_mask[:, -1] = True  # 屏蔽最后一个token（目标）
        self.no_mask = torch.zeros((1, self.context_size + 2), dtype=torch.bool)  # 不屏蔽任何token
        
        # 组合两种mask，用于批量索引
        self.all_masks = torch.cat([self.no_mask, self.goal_mask], dim=0)
        
        # 平均池化mask：用于在屏蔽目标时调整平均池化的权重
        # 当目标被mask时，需要重新归一化剩余token的权重
        self.avg_pool_mask = torch.cat([
            1 - self.no_mask.float(),  # 导航模式：所有token权重为1
            (1 - self.goal_mask.float()) * ((self.context_size + 2)/(self.context_size + 1))  # 探索模式：调整权重补偿被mask的目标
        ], dim=0)
    ...
    def forward(self, obs_img: torch.tensor, goal_img: torch.tensor, input_goal_mask: torch.tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # ========== 编码观测序列 ==========
        # 将观测图像按帧分割：[batch_size, 3*(context_size+1), H, W] -> (context_size+1) x [batch_size, 3, H, W]
        obs_img = torch.split(obs_img, 3, dim=1)
        # 拼接为一个大batch：[(context_size+1)*batch_size, 3, H, W]
